...
class AudioManager:
    ...
    def load_audio(self, indice):
        if not self._audio or indice >= len(self._audio):
            return
    ...
    def get_beat_multiplier(self):
        ...
        frame_atual = librosa.time_to_frames(tempo_ms / 1000.0, sr=self._sampler_rate, hop_length=512)
    ...
    def get_current_frequencies(self, num_bandas=10):
        ...
        if self._sampler_rate is None or self._stft is None:
        ...
        if frame_atual < self._stft.shape[1]:
